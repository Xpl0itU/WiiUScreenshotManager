...
APP_SHORTNAME	:= Screenshot Manager
...
CXXFLAGS	:=	$(CFLAGS) -std=c++20 -fpermissive -Wno-reorder
...
clean:
	@echo clean ...
	@rm -rf $(BUILD) $(TARGET).wuhb $(TARGET).rpx $(TARGET).elf ScreenshotManager *.zip

#-------------------------------------------------------------------------------
release: $(BUILD)
	@mkdir -p ScreenshotManager
	@cp ScreenshotManager.rpx ScreenshotManager
	@cp meta/hbl/icon.png ScreenshotManager
	@cp meta/hbl/meta.xml ScreenshotManager
	@zip -9 -r ScreenshotManager-HBL.zip ScreenshotManager
	@zip -9 ScreenshotManager-Aroma.zip ScreenshotManager.wuhb
	@rm -rf ScreenshotManager
#-------------------------------------------------------------------------------
else
.PHONY:	all
